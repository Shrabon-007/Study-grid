...
const TermMark = require("../models/term-mark.model");
const { determineCgpaAndInsights, getCourseMaxMarks, getGradeDetails } = require("../services/ai.service");
...
const getStudentAiCgpaContext = async (req, res) => {
  try {
    const ctx = await withContext(req);
    if (!ensureRole(res, ctx.user, ["student"])) return;
    if (!ctx.student) {
    ...
    const requestedSemester = normalize(req.query.semesterLabel || req.query.semester);
    const setups = await SemesterSetup.find({ studentId: ctx.student._id }).sort({ updatedAt: -1 });
    const activeSemester = requestedSemester || (setups[0] ? setups[0].semesterLabel : "Level-1 Term-1");
    const currentSetup = setups.find((s) => s.semesterLabel === activeSemester) || setups[0] || null;
    ...
    const [attendanceRows, ctRows, termMarkRows, pastSemesters] = await Promise.all([
      Attendance.find({ studentId: ctx.student._id, semesterLabel: activeSemester }),
      CtMark.find({ studentId: ctx.student._id, semesterLabel: activeSemester }),
      TermMark.find({ studentId: ctx.student._id, semesterLabel: activeSemester }),
      SemesterCgpa.find({ studentId: ctx.student._id }).sort({ updatedAt: -1 }),
    ]);

    const attendanceByCourseId = new Map();
    attendanceRows.forEach((row) => {
      if (row.courseId) attendanceByCourseId.set(String(row.courseId._id || row.courseId), row);
    });

    const ctByCourseId = new Map();
    ctRows.forEach((row) => {
      if (row.courseId) ctByCourseId.set(String(row.courseId._id || row.courseId), row);
    });

    const termMarksByCourseId = new Map();
    termMarkRows.forEach((row) => {
      if (row.courseId) termMarksByCourseId.set(String(row.courseId._id || row.courseId), row);
    });

    const courses = studentCourses
      .filter((sc) => sc.courseId)
      .map((sc) => {
        ...
        const maxes = getCourseMaxMarks(credit, courseType);

        const savedTerm = termMarksByCourseId.get(cId);
        const attRow = attendanceByCourseId.get(cId);
        const ctRow = ctByCourseId.get(cId);

        const attendanceMark = savedTerm
          ? savedTerm.attendanceMark
          : attRow && Number.isFinite(attRow.predictedMark)
          ? attRow.predictedMark
          : 0;

        const ctMark = savedTerm
          ? savedTerm.ctMark
          : ctRow && Number.isFinite(ctRow.total)
          ? ctRow.total
          : 0;

        const termFinalMark = savedTerm ? savedTerm.termFinalMark : 0;
        const totalMark = attendanceMark + ctMark + termFinalMark;
        const percentage = maxes.totalMax > 0 ? Number(((totalMark / maxes.totalMax) * 100).toFixed(1)) : 0;
        ...
        return {
          ...
          attendancePublished: attRow ? attRow.publishedByTeacher : false,
          ctMark,
          ctMax: maxes.ctMax,
          ctPublished: ctRow ? ctRow.publishedByTeacher : false,
          termFinalMark,
          termMax: maxes.termMax,
          totalMark,
          totalMax: maxes.totalMax,
          percentage,
          gradePoint: grade.point,
          letterGrade: grade.letter,
          isCustomSaved: Boolean(savedTerm),
        };
      });
    ...
    return res.status(200).json({
      ...
      data: {
        semesterLabel: activeSemester,
        targetCgpa: currentSetup ? currentSetup.targetCgpa : 3.5,
        totalCredit: currentSetup ? currentSetup.totalCredit : 0,
        pastSemesters: pastSemesters.map((s) => ({
    ...
    return res.status(500).json({
      success: false,
      message: "Could not load AI CGPA context.",
      error: error.message,
    });
  }
};

const saveStudentTermMarks = async (req, res) => {
  try {
    const ctx = await withContext(req);
    if (!ensureRole(res, ctx.user, ["student"])) return;
    if (!ctx.student) {
    ...
    const semesterLabel = normalize(req.body.semesterLabel);
    const items = Array.isArray(req.body.items) ? req.body.items : [];

    if (!semesterLabel || items.length === 0) {
      return res.status(400).json({ success: false, message: "semesterLabel and items are required." });
    }
    ...
    const updates = await Promise.all(
      items.map(async (item) => {
        ...
        const maxes = getCourseMaxMarks(course.credit, course.courseType);
        const attendanceMark = Math.max(0, Math.min(maxes.attendanceMax, Number(item.attendanceMark || 0)));
        const ctMark = Math.max(0, Math.min(maxes.ctMax, Number(item.ctMark || 0)));
        const termFinalMark = Math.max(0, Math.min(maxes.termMax, Number(item.termFinalMark || 0)));

        const totalMark = attendanceMark + ctMark + termFinalMark;
        const percentage = maxes.totalMax > 0 ? (totalMark / maxes.totalMax) * 100 : 0;
        const grade = getGradeDetails(percentage);

        return TermMark.findOneAndUpdate(
          { studentId: ctx.student._id, courseId: course._id, semesterLabel },
          {
            $set: {
              attendanceMark,
    ...
    return res.status(200).json({
      success: true,
      message: "Course marks saved successfully.",
      data: { count: updates.filter(Boolean).length },
    });
  } catch (error) {
    return res.status(500).json({
      success: false,
      message: "Could not save course marks.",
      error: error.message,
    });
  }
};

const evaluateStudentAiCgpa = async (req, res) => {
  try {
    const ctx = await withContext(req);
    if (!ensureRole(res, ctx.user, ["student"])) return;
    if (!ctx.student) {
    ...
    const targetCgpa = Number(req.body.targetCgpa || 3.5);

    const pastSemesters = await SemesterCgpa.find({
      studentId: ctx.student._id,
      semesterLabel: { $ne: semesterLabel },
    });
    ...
    return res.status(200).json({
      success: true,
      message: "CGPA evaluated successfully with AI.",
      data: result,
    });
  } catch (error) {
    return res.status(500).json({
      success: false,
      message: "Could not evaluate CGPA with AI.",
      error: error.message,
    });
  }
};
